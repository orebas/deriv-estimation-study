...
class PyNumDiffMethods(MethodEvaluator):
    ...
    def evaluate_method(self, method_name: str) -> Dict:
        ...
        Args:
            method_name: Name of method to evaluate. Supported:
                ...
                - "PyNumDiff-Spline-Auto": Spline with automatic parameters
                - "PyNumDiff-Spline-Tuned": Spline with tuned parameters
                ...
                - "PyNumDiff-TVRegularized-Auto": Total Variation Regularized (EXCELLENT!)
    ...
    def _splinediff(self, regime: str = "auto") -> Dict:
        """Spline smoothing (orders 0-1 only)."""
        t = self.x_train
        y = self.y_train
        n = len(t)
        dt = float(np.mean(np.diff(t))) if n > 1 else 1.0
        ...
        try:
            if regime == "auto":
                ...
                opt = self._optimize_params(splinediff, y, dt)
                if opt is None:
                    return {"predictions": {}, "failures": {"error": "splinediff_optimize_failed"}}
                x_smooth, dx_dt = splinediff(y, dt, **opt["params"])
                meta = {"method": "splinediff", "regime": regime, "dt": dt,
                        "opt_params": opt["params"], "opt_value": opt["value"], "tvgamma": opt["tvgamma"]}
            else:  # tuned
                from pynumdiff.polynomial_fit import splinediff
                x_smooth, dx_dt = splinediff(y, dt, degree=3, s=1, num_iterations=1)
                meta = {"method": "splinediff", "regime": regime, "dt": dt, "degree": 3, "s": 1}
    ...
    # =========================================================================
    # ADDITIONAL METHODS (Orders 0-1 only)
    # =========================================================================
